...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "iqp")) != -1) {
      //opt is the argument, taken from the shell, valid is i and p.
    switch (opt) {
    case 'q':
      quietFlag = 1;
      break;
 ...
 */
static void DumpPathnameChecksum(struct unixfilesystem *fs, FILE *f) {
  DumpPathAndChildren(fs, "/", ROOT_INUMBER, f);//filesystem , / , 1 , stdout
}
...
static int GetDirEntries(struct unixfilesystem *fs, int inumber, struct direntv6 *entries, int maxNumEntries) {
  struct inode in;
  int err = inode_iget(fs, inumber, &in);//get current directory
  if (err < 0) return err;
